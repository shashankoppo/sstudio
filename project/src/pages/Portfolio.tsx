...
export const Portfolio = ({ onNavigate }: PortfolioProps) => {
  ...
  const categories = ['All', 'Weddings', 'Commercial', 'Fashion', 'Events', 'Cinematic'];
              ...
              {filteredItems.map((item, idx) => (
                <AnimatedSection key={item.id} delay={idx * 50}>
                  <div onClick={() => setSelectedImage(item)} className="group relative h-80 overflow-hidden rounded-2xl cursor-pointer">
                    <img src={item.imageUrl} alt={item.altText} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy" />
        ...
        {selectedImage && (
          <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
            ...
            <div className="max-w-4xl w-full space-y-6">
              <img src={selectedImage.imageUrl} alt={selectedImage.altText} className="w-full h-96 object-cover rounded-2xl" />
